    ...
    <div className='p-4 flex items-center shadow-xl shadow-zinc-400 rounded-md justify-center flex-col xs:w-11/12 md:w-6/12 xl:w-4/12 mt-[5%]'>
        <div className='shadow-2xl w-full bg-white p-3 mb-4 rounded-sm shadow-zinc-300'>
            ...
            <div className='flex items-center justify-start'>
                ...
                <button disabled={bool} onClick={AddTask} className={`${loading ? 'bg-zinc-400' : 'bg-blue-800'} w-3/12 text-center flex items-center justify-center  hover:bg-blue-700 text-sm font-Quicksand p-2 hover:tracking-wider rounded-md text-white`}>{bool  ? <ScaleLoader color={'#fff'} loading={loading} size={3} aria-label="Loading Spinner" data-testid="loader" /> : 'Submit'}</button>
            </div>
        </div>
        {
